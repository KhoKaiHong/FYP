...
function OrganiserProfile() {
  ...
  return (
    <Card class="bg-muted shadow-none border-none">
      <CardHeader>
        <CardTitle class="text-xl">Organiser Profile</CardTitle>
      </CardHeader>
      <CardContent>
        <Show when={user()} keyed>
          {(loggedInOrganiser) =>
            loggedInOrganiser.role === "Organiser" ? (
              <div class="grid grid-cols-1 md:grid-cols-3 gap-y-3 md:gap-x-6 items-center justify-center">
                <div>
                  <p class="flex font-medium h-9 items-center">Organiser ID:</p>
                  <p class="flex h-9 items-center">{loggedInOrganiser.id}</p>
                </div>

                <div>
                  <p class="flex font-medium h-9 items-center">Name:</p>
                  <div class="flex items-center justify-between gap-x-2">
                    <p>{loggedInOrganiser.name}</p>
                    <ChangeNameDialog />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="flex items-center justify-between gap-x-2">
                    <p>{loggedInOrganiser.phoneNumber}</p>
