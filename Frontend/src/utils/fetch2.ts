...
export async function fetchWithAuth<T = any>({
  ...
  const makeRequest = async (token: string): Promise<ApiResult<T>> => {
    try {
      ...
      if (
        response.status === 401 &&
        parsedError.error === "ACCESS_TOKEN_EXPIRED"
      ) {
        // If access token expired, try to refresh it
        return await handleTokenRefresh(token, refreshToken);
      } else if (
        (response.status === 401 && parsedError.error === "SESSION_EXPIRED") ||
        (response.status === 403 && parsedError.error === "NO_AUTH")
      ) {
    ...
    try {
      ...
      if (
        (refreshResponse.status === 401 &&
          parsedError.error === "SESSION_EXPIRED") ||
        (refreshResponse.status === 403 && parsedError.error === "NO_AUTH")
      ) {
        localStorage.removeItem("accessToken");
        localStorage.removeItem("refreshToken");
      }
